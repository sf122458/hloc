...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        name = self.names[idx]
        image = read_image(self.root / name, self.conf.grayscale) # 读取图像
...
@torch.no_grad()
def extract_feature_from_query(
    conf: Dict,
    image_dir: Path,
    query_name: str,
    model: None,
    ...
    process_conf = SimpleNamespace(**{**default_conf, **conf})
    
    image = read_image(image_dir / query_name, process_conf.grayscale)
    image = image.astype(np.float32)
    size = image.shape[:2][::-1]
    
    if process_conf.resize_max and (
        process_conf.resize_force or max(size) > process_conf.resize_max
    ):
        scale = process_conf.resize_max / max(size)
        size_new = tuple(int(round(x * scale)) for x in size)
        image = resize_image(image, size_new, process_conf.interpolation)
        
    if process_conf.grayscale:
        image = image[None]
    else:
        image = image.transpose((2, 0, 1))
    image = image / 255.0
    
    image = torch.Tensor(image).to(device, non_blocking=True).unsqueeze(0)
    ...
    pred["image_size"] = original_size = data["original_size"][0]
    if "keypoints" in pred:
        ...
        uncertainty = getattr(model, "detection_noise", 1) * scale.mean()
